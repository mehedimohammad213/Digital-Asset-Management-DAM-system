...
test('discover full flow partial', async ({ page }) => {
  ...
  const fillField = async (label: string, value: string) => {
    await dialog.locator('div').filter({
      has: page.locator('p').filter({ hasText: new RegExp(`^${label}$`, 'i') }),
    }).getByRole('textbox').first().fill(value);
  };

  await fillField('Title', 'Automation QA Engineer');
  const typeCombo = dialog.locator('div').filter({ has: page.locator('p').filter({ hasText: /^Type$/i }) }).getByRole('combobox').first();
  ...
  await dialog.locator('div').filter({ has: page.getByText(/Automated Testdata/i) }).getByRole('checkbox').check();
  ...
  console.log('After save lines:', bodyAfterSave.split('\n').filter(l => /automation|sample|video|error/i.test(l)));
