...
test('discover form input structure', async ({ page }) => {
  ...
  const structure = await page.evaluate(() => {
    ...
    const texts = ['Title', 'Type', 'Date Time', 'Description', 'Tags', 'Automated Testdata', 'Hyperlink'];
    for (const t of texts) {
      ...
      if (el) {
        const parent = el.closest('[class*="field"], [class*="form"], div')?.parentElement ?? el.parentElement;
        const inputs = parent?.querySelectorAll('input:not([type="checkbox"]):not([type="file"]), textarea, select, [role="combobox"]');
        result.push({
          label: t,
  ...
  // Try filling fields
  const titleInput = page.locator('input').filter({ has: page.locator('xpath=..') }).nth(0);
  console.log('All visible inputs:');
  const allInputs = await page.locator('input:visible, textarea:visible, [role="combobox"]:visible').all();
